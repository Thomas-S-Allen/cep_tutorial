...
The variables we are interested in are categorical, so we will use a table of frequencies and a bar chart to eplore the relationship between `Type` and `Region`.
...
\end{aligned}
$$
For the "east" region, since there are `r df_counts[df_counts$Type == "disk" & df_counts$Region == "east","counts"]` stars with a disk and `r df_counts[df_counts$Type == "no disk" & df_counts$Region == "east" ,"counts"]` without, $\hat{p} = \frac{`r df_counts[df_counts$Type == "disk" & df_counts$Region == "east","counts"]`}{`r df_counts[df_counts$Type == "disk" & df_counts$Region == "east","counts"]` + `r df_counts[df_counts$Type == "no disk" & df_counts$Region == "east" ,"counts"]`} = 0.44$:
$$
\begin{aligned}
...
```
Therefore, we think that a plausable range of values that contain the population proportion in the east region is, $p_{east}$, is $0.39 - 0.49$, and for the west, $p_{west}$, is $0.48 - 0.6$.
...
chi_sq_stat
```
If $R$ is the number of categories in the response variable, and $C$ is the number of categories in the explanetory variable, then the degrees of freedom for a $X^{2}$ test are given by:
...
Can refer to the [summary tables](https://reed-statistics.github.io/math141f20/inference_procedures.html).  
